(() => {
  ...
  function confirmStatus() { if (!statusTarget) return; const nextStatus = $('managementStatusSelect').value, reason = clean($('managementStatusReason').value); if (['canceled', 'rejected', 'archived'].includes(nextStatus) && reason.length < 3) { alert('Informe uma justificativa com pelo menos 3 caracteres.'); return; }
  ...
  async function addAttachments(targetId, files) { if (!files || !files.length) return; const selected = [...files]; if (selected.some(file => file.size > MAX_ATTACHMENT_BYTES)) { alert('Cada anexo deve ter no máximo 5 MB para manter o backup local utilizável.'); return; }
    const state = read(); if (!state) return; const management = managementOf(state);
