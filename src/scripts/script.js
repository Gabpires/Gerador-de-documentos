(() => {
  ...
  async function exportarBackupProtegido() {
    ...
    try {
      ...
      const envelope = {
        app: 'Gerador de Documentos - Paraíba Imóveis', version: SCHEMA_VERSION, encrypted: true,
  ...
  function normalizarBackup(payload) {
    ...
    const history = fonte.history.map(r => {
      ...
      return {
        ...d, type: 'receipt', templateVersion: 1, letterhead: 'institutional', fields: { ...d }, amountWords: ajustarAcentos(valorExtenso(d.amount)), status,
    ...
    return {
      counters, documentCounters, history: [...unicos.values()], documents, draftDocuments, templates, contacts, clients, draft: normalizarRascunho(fonte.draft), management: fonte.management && typeof fonte.management === 'object' ? fonte.management : {}, meta: metaNormalizada(fonte.meta),
